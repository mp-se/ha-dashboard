...
describe("configValidator", () => {
  ...
  describe("validateConfig - app section", () => {
    it("should validate app.title is string", () => {
      const config = {
        app: { title: 123 },
        views: [
          { name: "view1", label: "View 1", icon: "mdi:home", entities: [] },
        ],
      };
      const result = validateConfig(config);
    ...
    it("should accept valid app title", () => {
      ...
      const result = validateConfig(config);
      expect(
        result.errors.filter((e) => e.message.includes("title")).length,
      ).toBe(0);
    });
  });
  ...
  describe("validateConfig - views", () => {
    ...
    it("should require entities array or undefined", () => {
      ...
      const result = validateConfig(config);
      expect(
        result.errors.some((e) => e.message.includes('invalid "entities"')),
      ).toBe(true);
    });
    ...
    it("should accept valid view", () => {
      const config = {
        app: { title: "Dashboard" },
        views: [
          { name: "home", label: "Home", icon: "mdi:home", entities: [] },
        ],
      };
      const result = validateConfig(config);
    ...
    it("should reject hidden attribute if not boolean", () => {
      ...
      const result = validateConfig(config);
      expect(result.errors.some((e) => e.message.includes("hidden"))).toBe(
        true,
      );
    });
  });
  ...
  describe("validateConfig - entities", () => {
    it("should validate entity with entity property", () => {
      ...
      const result = validateConfig(config);
      expect(
        result.errors.filter((e) => e.message.includes("sensor.test")).length,
      ).toBe(0);
    });
    ...
    it("should reject unknown component type", () => {
      ...
      const result = validateConfig(config);
      expect(
        result.errors.some((e) => e.message.includes("Unknown component type")),
      ).toBe(true);
    });
    ...
    it("should skip entity requirement when getter provided", () => {
      ...
      expect(
        result.errors.filter((e) => e.message.includes("missing required prop"))
          .length,
      ).toBe(0);
    });
  });
        ...
        views: [
          {
            ...
            icon: "mdi:home",
            entities: [
              { type: "HaImage", url: "https://example.com/image.jpg" },
            ],
          },
        ],
      };
  ...
  describe("validateConfig - conditional entities", () => {
    ...
    it("should require operator for HaWarning", () => {
      ...
      const result = validateConfig(config);
      expect(result.errors.some((e) => e.message.includes("operator"))).toBe(
        true,
      );
    });
  });
  ...
  describe("validateConfig - optional props", () => {
        ...
        views: [
          {
            ...
            icon: "mdi:home",
            entities: [
              { entity: "sensor.test", attributes: { min: 0, max: 100 } },
            ],
          },
        ],
      };
    ...
    it("should reject unknown props", () => {
      ...
      const result = validateConfig(config);
      expect(
        result.errors.some((e) => e.message.includes("unknown prop")),
      ).toBe(true);
    });
  });
  ...
  describe("validateConfig - complex scenarios", () => {
    ...
    it("should handle mixed valid and invalid entities", () => {
      ...
      const result = validateConfig(config);
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.message.includes("unknownProp"))).toBe(
        true,
      );
    });
