...
export function validateProperty(
  ...
  if (
    prop.type === "text" &&
    prop.maxLength &&
    String(value).length > prop.maxLength
  ) {
    return {
      valid: false,
      error: `${prop.label} must be ${prop.maxLength} characters or less`,
    };
  }
  ...
  if (
    prop.type === "text" &&
    prop.pattern &&
    !new RegExp(prop.pattern).test(String(value))
  ) {
  ...
  if (prop.type === "number") {
    if (prop.min !== undefined && Number(value) < prop.min) {
      return {
        valid: false,
        error: `${prop.label} must be at least ${prop.min}`,
      };
    }
    if (prop.max !== undefined && Number(value) > prop.max) {
      return {
        valid: false,
        error: `${prop.label} must be at most ${prop.max}`,
      };
    }
  }
