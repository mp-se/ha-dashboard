...
const VALID_MDI_ICONS = new Set<string>(
  Object.keys(mdiIcons)
    .filter((key) => {
      ...
      return typeof (mdiIcons as Record<string, unknown>)[key] === "string";
    })
    .map((key) => {
 ...
 */
export function isValidMdiIcon(iconName: unknown): boolean {
 ...
 */
export function suggestMdiIcons(iconName: unknown): string[] {
  if (!iconName || typeof iconName !== "string") {
    return [];
  }
 ...
 */
export function getMdiIconCount(): number {
