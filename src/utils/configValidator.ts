...
function validateEntity(
  ...
  if (entityConfig.entity) {
    const entityVal = Array.isArray(entityConfig.entity)
      ? entityConfig.entity[0]
      : entityConfig.entity;
    entityId = String(entityVal || "unknown");
    ...
    entityId = String(entityConfig.name || entityConfig.type);
  } else {
  ...
  const componentType =
    (entityConfig.type as string) ||
    getDefaultComponentType(
      entityConfig.entity as string | unknown[],
      entityConfig.getter as string,
    );
  ...
  if (componentType === "HaSensorGraph" && entityConfig.secondEntity) {
    errors.push({
      message:
        `⚠️  DEPRECATED: Component 'HaSensorGraph' in view '${viewName}' entity '${entityId}' uses deprecated 'secondEntity' prop. ` +
...
export function validateConfig(config: unknown): ValidationResult {
  ...
  if (!Array.isArray(cfg.views)) {
    errors.push({
      message: 'Configuration must have a "views" array',
      line: 10,
    });
    return {
      valid: false,
  ...
  // Validate each view
  for (let viewIndex = 0; viewIndex < cfg.views.length; viewIndex++) {
    const view = cfg.views[viewIndex] as Record<string, unknown>;
    if (!view.name) {
      errors.push({
        message: `View ${viewIndex} must have a "name" property`,
        line: 12 + viewIndex,
      });
      continue;
    }
    ...
    if (Array.isArray(view.entities)) {
      for (
        let entityIndex = 0;
        entityIndex < view.entities.length;
        entityIndex++
      ) {
