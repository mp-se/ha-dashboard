...
describe("useHaStore", () => {
  ...
  describe("loadLocalData", () => {
    it("should load local data from JSON file", async () => {
      ...
      const mockLocalData = {
        sensors: [{ entity_id: "sensor.local_1", state: "value1" }],
        devices: [{ id: "device1", name: "Local Device" }],
      };
    ...
    it("should provide helpful error for malformed JSON", async () => {
      ...
      global.fetch.mockImplementation(() =>
        Promise.resolve({
          ok: true,
          text: () =>
            Promise.resolve('{"app": {"title": "Test"}, invalid json'),
        }),
      );
  ...
  describe("Entity filtering methods", () => {
    ...
    it("should filter battery sensors by device_class", () => {
      ...
      expect(
        batteries.every((b) => b.attributes.device_class === "battery"),
      ).toBe(true);
    });
    ...
    it("should filter WiFi sensors by icon", () => {
      ...
      store.sensors = [
        {
          entity_id: "sensor.wifi_1",
          state: "-50",
          attributes: { icon: "mdi:wifi" },
        },
        {
          entity_id: "sensor.wifi_2",
          state: "-60",
          attributes: { icon: "mdi:wifi-low" },
        },
        {
          entity_id: "sensor.temp",
          state: "23",
      ...
      expect(
        wifiSensors.every((w) => w.attributes.icon.startsWith("mdi:wifi")),
      ).toBe(true);
    });
  ...
  describe("WebSocket registry operations", () => {
    ...
    describe("mapEntitiesToDevices", () => {
      it("should map entities to devices when device_id matches", () => {
        ...
        store.areas = [{ area_id: "area1", name: "Living Area", entities: [] }];
      ...
      it("should map devices to areas via area_id", () => {
        ...
        store.areas = [{ area_id: "area1", name: "Bedroom", entities: [] }];
        ...
        store.sensors = [
          {
            entity_id: "light.bedroom",
            state: "on",
            attributes: { device_id: "unknown-device" },
          },
        ];
        store.devices = [];
        store.areas = [];
      ...
      it("should not duplicate entities in device or area lists", () => {
        ...
        expect(
          store.devices[0].entities.filter((e) => e === "light.bedroom").length,
        ).toBe(1);
        expect(
          store.areas[0].entities.filter((e) => e === "light.bedroom").length,
        ).toBe(1);
      });
        ...
        store.areas = [
          {
            area_id: "area1",
            name: "Living Room",
            entities: ["light.bedroom", "light.living_room"],
          },
        ];
