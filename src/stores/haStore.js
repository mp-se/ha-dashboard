...
export const useHaStore = defineStore("haStore", () => {
      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          setTimeout(() => {
            if (firstUpdate) {
              console.warn(
                "subscribeEntities did not receive initial data within 5 seconds",
              );
              resolve();
            }
          }, 5000);
  ...
  const fetchDevices = async () => {
    ...
    if (isLocalMode.value) return;
    console.log(
      "fetchDevices: Devices will be fetched from WebSocket connection",
    );
  };
  ...
  const fetchEntityRegistry = async () => {
    ...
    try {
      ...
      console.log(
        `Fetched ${entityRegistry.length} entities from entity registry`,
      );
  ...
  const fetchAreaRegistry = async () => {
    ...
    try {
      ...
      console.log(
        `Fetched ${areasArray.length} areas from websocket:`,
        areasArray,
      );
      ...
      if (areasArray.length > 0) {
        ...
        for (const area of areasArray) {
          ...
          if (
            !sensors.value.find((s) => s.entity_id === areaEntity.entity_id)
          ) {
  ...
  const mapEntitiesToDevices = () => {
    ...
    for (const device of devices.value) {
      if (device.area_id && areasMap.has(device.area_id)) {
        ...
        const virtualAreaEntity = sensors.value.find(
          (s) => s.entity_id === `area.${device.area_id}`,
        );
    ...
    if (!(supportedFeatures & forecastCapabilities[forecastType])) {
      ...
      if (!alternativeType) {
        forecastErrors.value[entityId] =
          `Entity does not support ${forecastType} forecasts`;
        return;
      }
  ...
  const unsubscribeFromForecast = (entityId) => {
    ...
    if (typeof unsubscribeFn === "function") {
      try {
        ...
        console.error(
          `Error unsubscribing from forecast for ${entityId}:`,
          error,
        );
      }
    }
  ...
  const connectWebSocket = async () => {
    ...
    try {
      console.log(
        "Connecting to Home Assistant via websocket library:",
        haUrl.value,
      );
    ...
    try {
      ...
      try {
        ...
        console.error("[CONFIG] JSON parse error details:", {
          ...
          rawTextSample:
            typeof rawText === "string" ? rawText.substring(0, 100) : rawText,
        });
      ...
      console.log(
        "Config loaded successfully. Views:",
  ...
  const loadCredentials = async () => {
    ...
    if (dashboardConfig.value?.haConfig) {
      console.log("Checking haConfig:", {
        ...
        accessToken: dashboardConfig.value.haConfig.accessToken
          ? "present"
          : "missing",
      });
      if (
        dashboardConfig.value.haConfig.haUrl &&
        dashboardConfig.value.haConfig.accessToken
      ) {
  ...
  const init = async () => {
    ...
    try {
      ...
      if (
        configValidationError.value?.length > 0 &&
        configValidationError.value.some((err) => err.message?.includes("JSON"))
      ) {
        console.log(
          "Step 1: JSON syntax error found - not prompting for credentials",
        );
      ...
      if (isLocalMode.value) {
        ...
        console.log(
          "Step 3: Websocket connection established and data loaded...",
        );
        isInitialized.value = true;
        isLoading.value = false;
      ...
      if (error.message.includes("CORS error")) {
        ...
        lastError.value =
          "Failed to connect to Home Assistant. Check console for details.";
      }
    }
  };
    ...
    const promise = (async () => {
      try {
        ...
        if (!ct.includes("application/json")) {
          const txt = await res.text();
          throw new Error(
            "History endpoint did not return JSON. Response snippet: " +
              txt.slice(0, 300),
          );
        }
  ...
  async function fetchEnergyHistory(entityId, days = 1) {
    ...
    const promise = (async () => {
      try {
        ...
        let bucketMinutes;
        if (days <= 1)
          bucketMinutes = 60; // Hourly for 1 day
        else if (days <= 3)
          bucketMinutes = 1440; // Daily for 3 days
        else if (days <= 7)
          bucketMinutes = 1440; // Daily for 7 days
