...
describe('HaEnergy.vue', () => {
  ...
  it('should auto-detect energy consumption sensor', async () => {
    ...
    expect(wrapper.vm.energySensor.entity_id).toBe('sensor.accumulated_consumption_hemma');
  });

  it('should fallback to power sensor when energy sensor not available', async () => {
    store.sensors = store.sensors.filter((s) => s.attributes.device_class !== 'energy');
  ...
  it('should fetch energy history on mount', async () => {
    ...
    expect(store.fetchEnergyHistory).toHaveBeenCalledWith('sensor.accumulated_consumption_hemma', 1);
  });
  ...
  it('should refetch energy history when period changes', async () => {
    ...
    expect(store.fetchEnergyHistory).toHaveBeenCalledWith('sensor.accumulated_consumption_hemma', 3);
  });
  ...
  it('should display error message when fetch fails', async () => {
    store.fetchEnergyHistory = vi.fn().mockRejectedValue(new Error('API Error'));
