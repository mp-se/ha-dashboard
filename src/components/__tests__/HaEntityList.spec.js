...
describe("HaEntityList.vue", () => {
  ...
  beforeEach(() => {
    ...
    store.sensors = [
      ...
      },
      {
        ...
        state: "60",
        attributes: { friendly_name: "Humid" },
      },
      {
  ...
  describe("Domain mapping", () => {
    it("should map switch domain to HaSwitch", () => {
      const wrapper = mount(HaEntityList, {
        ...
        global: {
          plugins: [pinia],
          stubs: { HaSpacer: true, HaSensor: true, HaSwitch: true },
        },
      });
      expect(wrapper.vm.getComponentForDomain("switch.outlet")).toBe(
        "HaSwitch",
      );
    });
    ...
    it("should map binary_sensor domain to HaBinarySensor", () => {
      ...
      expect(wrapper.vm.getComponentForDomain("binary_sensor.motion")).toBe(
        "HaBinarySensor",
      );
    });
    ...
    it("should map alarm_control_panel domain to HaAlarmPanel", () => {
      ...
      expect(wrapper.vm.getComponentForDomain("alarm_control_panel.home")).toBe(
        "HaAlarmPanel",
      );
    });
    ...
    it("should map weather domain to HaWeather", () => {
      ...
      expect(wrapper.vm.getComponentForDomain("weather.home")).toBe(
        "HaWeather",
      );
    });
    ...
    it("should map update domain to HaBinarySensor", () => {
      ...
      expect(wrapper.vm.getComponentForDomain("update.device")).toBe(
        "HaBinarySensor",
      );
    });
    ...
    it("should map device_tracker domain to HaSensor", () => {
      ...
      expect(wrapper.vm.getComponentForDomain("device_tracker.phone")).toBe(
        "HaSensor",
      );
    });
    ...
    it("should map media_player domain to HaMediaPlayer", () => {
      ...
      expect(wrapper.vm.getComponentForDomain("media_player.tv")).toBe(
        "HaMediaPlayer",
      );
    });
    ...
    it("should map button domain to HaButton", () => {
      ...
      expect(wrapper.vm.getComponentForDomain("button.restart")).toBe(
        "HaButton",
      );
    });
    ...
    it("should return HaSensor for unknown domain", () => {
      ...
      expect(wrapper.vm.getComponentForDomain("unknown.entity")).toBe(
        "HaSensor",
      );
    });
  ...
  describe("Entity processing - direct entityId", () => {
    it("should render entity with directentityId from store", () => {
      ...
      expect(wrapper.vm.displayedEntities[0].entity.entity_id).toBe(
        "sensor.temp",
      );
    });
  ...
  describe("Entity processing - getter functions", () => {
      ...
      store.getBatterySensors = vi.fn().mockReturnValue([
        {
          ...
          state: "80",
          attributes: { friendly_name: "Battery 1" },
        },
        {
    ...
    it("should support custom component override for getter results", () => {
      ...
      expect(wrapper.vm.displayedEntities).toHaveLength(2);
      expect(wrapper.vm.displayedEntities[0].component).toBe(
        "BatteryComponent",
      );
      expect(wrapper.vm.displayedEntities[1].component).toBe(
        "BatteryComponent",
      );
    });
  ...
  describe("Spacer handling", () => {
    ...
    it("should keep spacers in filtered output", () => {
      ...
      expect(wrapper.vm.displayedEntities[2].entity.entity_id).toBe(
        "sensor.humid",
      );
    });
  });
  ...
  describe("Error handling and filtering", () => {
    ...
    it("should filter out null entities", () => {
      ...
      const hasNullEntity = wrapper.vm.displayedEntities.some(
        (item) => !item.isSpacer && !item.entity,
      );
  ...
  describe("Mixed entity types", () => {
    it("should handle mix of entityId and getter", () => {
      ...
      expect(wrapper.vm.displayedEntities[1].entity.entity_id).toBe(
        "sensor.battery",
      );
      expect(wrapper.vm.displayedEntities[2].entity.entity_id).toBe(
        "sensor.humid",
    ...
    it("should handle mix of entities with and without spacers", () => {
      ...
      expect(wrapper.vm.displayedEntities).toHaveLength(3);
      const spacers = wrapper.vm.displayedEntities.filter(
        (item) => item.isSpacer,
      );
  ...
  describe("Update prop reactivity", () => {
    it("should update displayed entities when prop changes", async () => {
      ...
      await wrapper.setProps({
        entities: [{ entityId: "sensor.temp" }, { entityId: "sensor.humid" }],
      });
