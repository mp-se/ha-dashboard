...
describe('JsonConfigView.vue', () => {
  ...
  it('uses default component type when not explicitly specified', () => {
    ...
    expect(useDefaultComponentType.getDefaultComponentType).toHaveBeenCalledWith(
      'sensor.temperature',
      undefined
    );
  ...
  it('passes attributes to components', () => {
    ...
    expect(sensor.props('attributes')).toEqual(['unit_of_measurement', 'device_class']);
  });
      ...
      views: [
        {
          ...
          entities: [
            {
              type: 'HaEntityList',
              entities: [{ entity: 'sensor.temp' }, { entity: 'sensor.humidity' }],
              componentMap: { default: 'HaSensor' },
              attributes: [],
            },
          ],
