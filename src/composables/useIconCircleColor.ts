 ...
 *
 * @param entity - The entity object with state and attributes
 * @param entityId - The entity ID string for domain extraction
 * @returns Hex color code (e.g., '#28a745' for green, '#dc3545' for red)
 */
...
export const useIconCircleColor = (
  ...
  if (deviceClass === "voltage" || /V$|volt/i.test(unit)) {
    ...
    if (!Number.isNaN(value)) {
      ...
      if (value >= 180) {
        ...
        if ((value >= 10.5 && value < 12) || (value > 14.5 && value <= 15.2))
        ...
        if ((value >= 20 && value < 24) || (value > 29 && value <= 30.4))
          return "#fd7e14"; // Orange - Caution
        return "#dc3545"; // Red - Critical
  ...
  if (
    (deviceClass === "battery" ||
      (domain === "sensor" && entityId?.includes("battery"))) &&
    deviceClass !== "voltage"
  ) {
    const value = Number(state);
    if (!Number.isNaN(value)) {
  ...
  if (
    ...
    /wifi|signal|strength|rssi|dBm/i.test(unit)
  ) {
    const value = Number(state);
    if (!Number.isNaN(value)) {
      // Handle percentage
  ...
  if (
    deviceClass === "aqi" ||
    /co2|ppm|air.*quality|aqi|pm2\.5|pm10|ug\/m|µg\/m/i.test(unit)
  ) {
    const value = Number(state);
    if (!Number.isNaN(value)) {
